...
void dfs1(int v, const vector<list<int>>& graph, vector<bool>& visited, list<int>& finishOrder) {
    visited[v] = true;
    for (int u : graph[v]) {
        if (!visited[u]) {
...
void dfs2(int v, const vector<list<int>>& reverseGraph, vector<bool>& visited, vector<int>& component) {
    visited[v] = true;
